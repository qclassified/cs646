...
// ============== I/O Functions ================   
int log(const char *format, ...);                // log to monitor/file/both, replaces printf/fprintf
           ...
           // log("\n-- %d, %s{%s}%d - %d\n", icount, code, desc, c, mstot); //debug
            ...
            if ( *code == 'S'){
                if ( streq(desc, "begin") ){
                    ...
                    oson ++;
                    log("%.6f - Simulator program starting\n", now());
                }
                    ...
                    oson--;
                    log("%.6f - Simulator Program ending\n", now());
                }
            ...
            if ( *code == 'A' ){
                if ( streq(desc, "begin") ){
                    inproc++;
                    log("%.6f - OS: preparing process %d\n", now(), id);
                    pcb.state = START;
                    log("%.6f - OS: starting process %d\n", now(), id);
                    pcb.state = READY;
                } else if ( streq(desc, "finish")) {
                    inproc--;
                    log("%.6f - OS: removing process %d\n", now(), id);
                    pcb.state = EXIT;
                }
            } else if ( *code == 'P' ){
                pcb.state = RUNNING;
                log("%.6f - Process %d: start processing action\n", now(), id);
                wait(mstot);
                log("%.6f - Process %d: end processing action\n", now(), id);
                ...
                if (streq(desc, "block")){
                    log("%.6f - Process %d: start memory blocking\n", now(), id);
                    wait(mstot);
                    log("%.6f - Process %d: end memory blocking\n", now(), id);
                    ...
                    log("%.6f - Process %d: allocating memory\n", now(), id);
                    ...
                    wait(mstot);
                    log("%.6f - Process %d: memory allocated at 0x%08X\n", now(), id, loc);
                }
            }
...
void *wait_t(void *arg){
    ...
    if (curmax > -1) {
        log("%.6f - Process %d: start %s %s on %s %d\n", now(), procid, desc, temp, part, *cur);
        *cur = *cur + 1;
        if (*cur > curmax) *cur = 0;
    } else
        log("%.6f - Process %d: start %s %s\n", now(), procid, desc, temp);
   
    wait(msec);
    log("%.6f - Process %d: end %s %s\n", now(), procid, desc, temp);
