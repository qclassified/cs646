...
unsigned int printsem = 1;
unsigned int hddsem = 1;
unsigned int curprint = 0;
unsigned int curhdd = 0;

unsigned int monmut = 1;
unsigned int mousemut = 1;
unsigned int keybmut = 1;
...
struct Config getconfig(FILE *fconf){
    ...
    while (fgets(line, sizeof(line), fconf)){
        ...
        } else if (streq(key, "memory block size")){    conf.memblock = verify(val, "memory block", "size");
